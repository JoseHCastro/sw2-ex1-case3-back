...
function buildUserPayload({ diagram, intent, user_message }) {
  return JSON.stringify({
    ...
    user_message: (user_message && { text: user_message.text, image_url: user_message.imageBase64 }) || { text: '', image_url: '' }
  });
}
...
function buildSystemPrompt() {
  return [
    'Eres un agente experto en modelado de bases de datos relacionales utilizando diagramas de clase UML.',
...
function authHeaderVariants() {
  if (AGENT_EXTRA_ENDPOINT) {
    return [{ 'x-goog-api-key': AGENT_TOKEN },];
  } else {
    return [
...
function buildBodies(messages) {
  ...
  if (AGENT_EXTRA_ENDPOINT) {
    return [{
      ...
      body: messages[0], // { payload: messages } // enviamos mensajes también en el extra endpoint
    }];
  }
...
// ---------- Llamada principal ----------
async function callAgent({ diagram, intent = 'free_chat', user_message = { text: '', imageBase64: '', imageType: '' }, forceMock = false }) {
  if (AGENT_MOCK || forceMock) {
    ...
    return {
      analysis: {
        ...
        user_request: user_message.text,
      },
      proposal: {
        patch: {
          classes: [
    ...
    const messages = [
      {
        ...
        "contents": [{
          "parts": [
            { "text": "Sobre estas clases y relaciones del diagrama de clases UML actual: " + JSON.stringify(diagram || { classes: [], relations: [] }), },
            (user_message.imageBase64 &&
            {
              "inline_data": {
                "mime_type": user_message.imageType,
                "data": user_message.imageBase64.split(',')[1] // remove the "data:image/png;base64" string prepended to the image data in base64
              }
            }) || { "text": "" },
      ...
      try {
        ...
        if (object) {
          ...
          if (AGENT_EXTRA_ENDPOINT) {
            ...
            const txt = object.candidates[0].content.parts[0].text;
            const newObject = coerceAgentObjectFromText(txt, { intent });
            console.log("diagrama generado por IA (normalizado):");
            return newObject;
          }
          return object;
        }
