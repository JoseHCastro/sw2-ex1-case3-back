...
exports.obtenerDiagramasInvitado = async (req, res) => {
  ...
  try {
      ...
      include: [
        {
          ...
          include: [
            {
              model: Usuario,
              as: 'propietario', // Usar el alias correcto definido en el modelo
              attributes: ['nombre'],
            },
          ],
        },
    ...
    const diagramasInvitados = invitaciones.map(inv => ({
      ...
      propietarioNombre: inv.Diagrama?.propietario?.nombre || 'Propietario no encontrado',
    }));
  ...
  try {
    ...
    const invitacion = await DiagramaUsuario.findOne({
      ...
      include: [{ 
        ...
        include: [{ 
          model: Usuario, 
          as: 'propietario', // Usar el alias correcto
