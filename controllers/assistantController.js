...
async function analyze(req, res) {
  try {
    ...
    if ((!baseDiagram.titulo || baseDiagram.titulo === 'Sin título') && user_message) {
      baseDiagram.titulo = user_message.text.slice(0, 80);
    }

    // Llamar al agente de IA para procesar el diagrama
    const data = await callAgent({ diagram: baseDiagram, intent: intent || 'free_chat', user_message: user_message || {text: '', imageBase64: '', imageType: ''}, forceMock });
